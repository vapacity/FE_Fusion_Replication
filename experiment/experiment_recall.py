...
import FE_Net
# 数据库的路径和时间戳文件
database_dir = '/root/autodl-tmp/processed_data/dvs_vpr_2020-04-28-09-14-11'
gps_file = "/root/autodl-tmp/processed_data/dvs_vpr_2020-04-28-09-14-11/interpolated_gps.txt"
# 图像预处理
...
model.load_state_dict(torch.load("/root/FE_Fusion/train/result_2024-10-16/saved_model/model_epoch_10.pth"))
...
# print(database_gps_data)
# print(database_gps_data)
# # 存储生成的数据库特征和GPS信息
# np.save('/root/autodl-tmp/experiment/database_features.npy', database_features)
# np.save('/root/autodl-tmp/experiment/database_gps_data.npy', database_gps_data)
...
def recall_at_n_with_distance(query_loader, database_features, query_gps_data, database_gps_data, model, N=5, distance_threshold=75):
    ...
    with torch.no_grad():
        for frames, event_volumes, query_gps_info in tqdm(query_loader, desc="Processing queries"):
            frames, event_volumes = frames.cuda(), event_volumes.cuda()
            ...
            # 使用 torch.topk 来获取前N的索引 (前 N 大的相似度值及其索引)
            ...
            for i in range(query_features.size(0)):  # 遍历 batch 中的每个 query
                ...
                for idx in top_n_indices[i]:  # 遍历 top N 的索引
                    ...
                    print("databse:",idx,db_lat,db_lon)
                    # 计算地理距离
...
query_gps_file = "/root/autodl-tmp/processed_data/dvs_vpr_2020-04-24-15-12-03/interpolated_gps.txt"
